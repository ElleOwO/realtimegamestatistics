...
import { Target, Zap, Activity, Users, Shield, TrendingUp, Compass, HeartPulse, Swords, FastForward } from 'lucide-react';
...
export function TeamStatComparison() {
  ...
  const derived = useMemo(() => {
    ...
    const players0 = data.players.filter(p => p.team === 0);
    const players1 = data.players.filter(p => p.team === 1);

    const verticalCompactness = (players: typeof data.players) => {
      if (players.length === 0) return null;
      const ys = players.map(p => p.y_m);
      return Math.max(...ys) - Math.min(...ys);
    };

    const explosiveDistance = (players: typeof data.players) => {
      return players.reduce((sum, p) => sum + ((p.sprint_distance_km ?? 0) * 1000), 0);
    };

    const totalSprintCount = (players: typeof data.players) => {
      return players.reduce((sum, p) => sum + (p.sprint_count ?? 0), 0);
    };

    const totalDistanceM = (players: typeof data.players) => {
    ...
    const fieldTiltFromZones = () => {
      const t0 = data.zone_stats?.team0;
      const t1 = data.zone_stats?.team1;
      if (!t0 || !t1) return null;
      const t0Atk = (t0.attacking_third_count ?? 0) + (t0.Q2_atk_left ?? 0) + (t0.Q4_atk_right ?? 0);
      const t1Atk = (t1.attacking_third_count ?? 0) + (t1.Q2_atk_left ?? 0) + (t1.Q4_atk_right ?? 0);
    ...
    const xgPerPoss0 = xgPerPossession(data.total_xg_team0, data.possession.team0_pct);
    const xgPerPoss1 = xgPerPossession(data.total_xg_team1, data.possession.team1_pct);
  ...
  const territorialStats = [
    ...
    { 
      ...
      home: data?.deep_completions ?? '—',
      away: '—',
      homePercent: data?.deep_completions ? Math.min((data.deep_completions / 20) * 100, 100) : 50,
    ...
    { 
      ...
      homePercent: vcHome != null && vcAway != null ? (vcHome / (vcHome + vcAway)) * 100 : 50,
  ...
  const creativeStats = [
    { 
      ...
      homePercent: xgPerPossHome != null && xgPerPossAway != null
    ...
    { 
      ...
      home: data?.ppda ?? '—',
      away: '—',
      homePercent: data?.ppda ? Math.min((data.ppda / 20) * 100, 100) : 50,
      icon: <Swords className="w-3 h-3 text-primary" /> 
    },
    { 
      label: 'Packing Rate', 
      home: data?.packing_rate ?? '—',
      away: '—',
      homePercent: data?.packing_rate ? Math.min((data.packing_rate / 60) * 100, 100) : 50,
  ...
  const defensiveStats = [
    { 
      ...
      home: data?.attacking_third_recoveries ?? '—',
      away: '—',
      homePercent: data?.attacking_third_recoveries ? Math.min((data.attacking_third_recoveries / 10) * 100, 100) : 50,
      icon: <Shield className="w-3 h-3 text-primary" /> 
    },
    { 
      label: 'Def. Line Height (m)', 
      home: data?.defensive_line_height_m?.toFixed(1) ?? '—',
      away: '—',
      homePercent: data ? (data.defensive_line_height_m / 105) * 100 : 50,
      icon: <TrendingUp className="w-3 h-3 text-primary" /> 
    },
    { 
      label: 'Counter Threat', 
      home: data?.counter_attack_threat_score ?? '—',
      away: '—',
      homePercent: data?.counter_attack_threat_score ? Math.min((data.counter_attack_threat_score / 10) * 100, 100) : 50,
  ...
  const physicalStats = [
    { 
      ...
      homePercent: derived?.dist0 != null && derived?.dist1 != null
    ...
    { 
      ...
      homePercent: expDistHome != null && expDistAway != null
    ...
    { 
      ...
      homePercent: derived?.sprint0 != null && derived?.sprint1 != null
  ...
  return (
    <Card className="h-full border-border bg-card overflow-hidden flex flex-col">
      <CardHeader className="p-4 md:p-6 border-b border-border flex flex-row items-center justify-between bg-zinc-950/50">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <CardTitle className="text-xl font-black uppercase tracking-tight text-foreground italic">Dynamic Metrics</CardTitle>
          ...
          <div className="p-4 md:p-6 grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-10">
            <div className="space-y-4">
              <h3 className="text-xs font-black uppercase tracking-widest text-primary border-b border-zinc-800 pb-2 flex items-center gap-2">
                <Compass className="w-4 h-4" /> Territorial Control
              </h3>
              <div className="space-y-4">
                {territorialStats.map((stat) => (
                  <StatRow key={stat.label} label={stat.label} homeValue={stat.home} awayValue={stat.away} homePercent={stat.homePercent} icon={stat.icon} />
                ))}
              </div>
            </div>
            ...
            <div className="space-y-4">
              <h3 className="text-xs font-black uppercase tracking-widest text-blue-400 border-b border-zinc-800 pb-2 flex items-center gap-2">
                <Zap className="w-4 h-4" /> Creative Threat
              </h3>
              <div className="space-y-4">
            ...
            <div className="space-y-4">
              <h3 className="text-xs font-black uppercase tracking-widest text-yellow-500 border-b border-zinc-800 pb-2 flex items-center gap-2">
                <HeartPulse className="w-4 h-4" /> Physical Load
              </h3>
              <div className="space-y-4">
