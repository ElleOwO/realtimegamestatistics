...
export function PitchView({
  ...
  const viewBox = isPortrait ? "-0.2 -0.2 60.4 100.4" : "-0.2 -0.2 100.4 60.4";
      ...
      for (let i = 0; i < players.length; i++) {
        for (let j = i + 1; j < players.length; j++) {
          ...
          const dist = Math.sqrt(
            Math.pow(p1.x_m - p2.x_m, 2) + Math.pow(p1.y_m - p2.y_m, 2),
          );
          if (dist < 25) {
  ...
  return (
    <div className="relative w-full h-full flex items-center justify-center overflow-hidden">
      ...
      >
        <rect x="-1" y="-1" width={isPortrait ? 62 : 102} height={isPortrait ? 102 : 62} fill="#072a1b" />
