...
export default function MatchDetailPage() {
  ...
  const [artifactPolicy, setArtifactPolicy] = useState<"compact" | "source" | "full">("full");
  ...
  useEffect(() => {
    load();
    postgameApi.runtime().then((runtime) => setArtifactPolicy(runtime.artifact_policy)).catch(() => undefined);
  }, [load]);
  const onStream = useCallback((message: StreamMessage) => {
    ...
    if (message.type === "progress") {
      setMatch((current) => current?.latest_job ? {
        ...current,
        status: "running",
        latest_job: { ...current.latest_job, ...message.payload },
      } as Match : current);
    }
    ...
    if (["job_status", "completed", "error"].includes(message.type)) load();
  }, [load]);
  ...
  return (
    <div className="app-container app-page app-stack font-sans">
      ...
      ) : report ? <ReportWorkspace match={match} initialReport={report} artifactPolicy={artifactPolicy} onReload={async () => setReport(await postgameApi.report(id))} /> : <Loader2 className="h-6 w-6 animate-spin" />}
    </div>
  );
}
...
function ReportWorkspace({ match, initialReport, artifactPolicy, onReload }: { match: Match; initialReport: MatchReport; artifactPolicy: "compact" | "source" | "full"; onReload: () => Promise<void> }) {
  ...
  const hasVideo = match.source_codec !== "rtgs-observation" && artifactPolicy !== "compact";
...
function PitchSnapshot({ observations }: { observations: Observation[] }) { return <div className="mx-auto flex w-full max-w-sm flex-col lg:max-w-none"><p className="mb-3 text-[10px] font-black uppercase tracking-wider text-muted-foreground">Synchronized pitch observations</p><svg viewBox="0 0 68 105" className="aspect-[68/105] w-full rounded-xl bg-[#0a3523]"><rect x="0.5" y="0.5" width="67" height="104" fill="none" stroke="#ffffff66" /><line x1="0" x2="68" y1="52.5" y2="52.5" stroke="#ffffff66" /><circle cx="34" cy="52.5" r="9.15" fill="none" stroke="#ffffff66" />{observations.filter((item) => item.pitch_x_m !== null && item.pitch_y_m !== null).map((item) => <circle key={item.id} cx={68 - item.pitch_y_m!} cy={item.pitch_x_m!} r={item.object_type === "ball" ? 1.2 : 1.7} fill={item.object_type === "ball" ? "white" : item.team === "home" ? "#23a469" : "#DBCC52"} stroke="#111" strokeWidth="0.4" />)}</svg>{observations.length === 0 && <p className="mt-2 text-xs text-muted-foreground">No valid observations at this timestamp.</p>}</div>; }
