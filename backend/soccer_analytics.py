...
FIELD_DETECTION_MODEL_ID  = os.environ.get("FIELD_MODEL_ID",  "football-field-detection-f07vi/14")
...
def compute_xg(
    ...
    if xg_model is not None:
        ...
        dmatrix = xgb.DMatrix(
            features,
            feature_names=[
                "distance_m","angle_deg","in_box","under_pressure",
                "is_header","is_weak_foot","is_counter","centrality_m"
            ]
        )
...
def assign_zone(x_m: float, y_m: float) -> str:
    ...
    if defensive and left:     return "Q1_def_left"
    if not defensive and left: return "Q2_atk_left"
    if defensive:              return "Q3_def_right"
    return "Q4_atk_right"
...
def update_possession_log(
    ...
    def nearest_dist(xy):
        return float(np.min(np.linalg.norm(xy - ball, axis=1))) \
               if len(xy) > 0 else float("inf")
...
def compute_zone_stats(team_xy: np.ndarray) -> dict:
    ...
    """
    counts = {z: 0 for z in
              ("Q1_def_left", "Q2_atk_left", "Q3_def_right", "Q4_atk_right")}
    def_third = 0
    atk_third = 0
    for x_m, y_m in team_xy:
        counts[assign_zone(x_m, y_m)] += 1
        if x_m < -35.0: def_third += 1
        if x_m >  35.0: atk_third += 1
...
class PlayerStats:
    ...
    zone_frames: Dict[str, int] = field(default_factory=lambda: {
        "Q1_def_left": 0, "Q2_atk_left": 0,
        "Q3_def_right": 0, "Q4_atk_right": 0,
    })
    ...
    def pass_accuracy(self) -> float:
        if self.passes_attempted == 0: return 0.0
...
def update_player_registry(
    ...
    if delta_m < 10.0 and elapsed > 0:
        ...
        sprinting_now  = sum(1 for s in stats.speed_buffer
                             if s > SPRINT_THRESHOLD_MS)
        sprinting_prev = sum(1 for s in list(stats.speed_buffer)[:-2]
                             if s > SPRINT_THRESHOLD_MS)
        if sprinting_now >= 2:
            if sprinting_prev == 0:
...
def init_gemini():
    ...
    if not GEMINI_AVAILABLE:
        print("⚠️  google-genai not installed — AI insights disabled.  pip install google-genai")
        return None
    try:
...
def build_gemini_prompt(
    ...
    usask = sorted(
        ...
        key=lambda s: s.distance_km, reverse=True)[:4]
    player_lines = "\n".join([
...
def main():
    ...
    while True:
        ...
        if keypoints.xy is None or len(keypoints.xy) == 0 or keypoints.confidence is None:
        ...
        team0_xy = pitch_players_xy[t0_mask] if t0_mask.any() else np.array([]).reshape(0, 2)
        ...
        for i in range(len(merged)):
            if merged.tracker_id is None: continue
        ...
        ball_payload = (
            ...
            if len(pitch_ball_xy) > 0 else None
        )
        ...
        if len(pitch_ball_xy) > 0:
            pitch_img = draw_points_on_pitch(
                CONFIG, pitch_ball_xy, sv.Color.WHITE, sv.Color.BLACK, 10, pitch=pitch_img)
        if t0_mask.any():
            pitch_img = draw_points_on_pitch(
                CONFIG, pitch_players_xy[t0_mask],
                sv.Color.from_hex("00BFFF"), sv.Color.BLACK, 16, pitch=pitch_img)
        if t1_mask.any():
            pitch_img = draw_points_on_pitch(
                CONFIG, pitch_players_xy[t1_mask],
                sv.Color.from_hex("FF1493"), sv.Color.BLACK, 16, pitch=pitch_img)
        if len(pitch_refs_xy) > 0:
            pitch_img = draw_points_on_pitch(
                CONFIG, pitch_refs_xy,
                sv.Color.from_hex("FFD700"), sv.Color.BLACK, 16, pitch=pitch_img)

        # Voronoi
        if t0_mask.any() and t1_mask.any():
            vbase = draw_pitch(CONFIG, background_color=sv.Color.WHITE,
                               line_color=sv.Color.BLACK)
            vimg  = draw_pitch_voronoi_diagram_2(
                CONFIG, pitch_players_xy[t0_mask], pitch_players_xy[t1_mask],
                sv.Color.from_hex("00BFFF"), sv.Color.from_hex("FF1493"), pitch=vbase)
            if len(pitch_ball_xy) > 0:
                vimg = draw_points_on_pitch(
                    CONFIG, pitch_ball_xy, sv.Color.WHITE, sv.Color.WHITE,
                    8, thickness=1, pitch=vimg)
            vimg = draw_points_on_pitch(
                CONFIG, pitch_players_xy[t0_mask],
                sv.Color.from_hex("00BFFF"), sv.Color.WHITE, 16, thickness=1, pitch=vimg)
            vimg = draw_points_on_pitch(
                CONFIG, pitch_players_xy[t1_mask],
                sv.Color.from_hex("FF1493"), sv.Color.WHITE, 16, thickness=1, pitch=vimg)
        ...
        for i, line in enumerate([
            ...
            cv2.putText(ann, line, (10, 30 + i * 28),
