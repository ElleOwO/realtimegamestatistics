...
"$RTGS_DEV_VENV/bin/python" -m compileall -q "$RTGS_ROOT/scripts" "$RTGS_ROOT/backend"
...
bash -n "$RTGS_ROOT/rtgs" "$RTGS_ROOT"/scripts/local/*.sh "$RTGS_ROOT"/scripts/runpod/*.sh \
  "$RTGS_ROOT"/scripts/relay/*.sh "$RTGS_ROOT"/deploy/runpod/*.sh "$RTGS_ROOT"/deploy/control/*.sh
