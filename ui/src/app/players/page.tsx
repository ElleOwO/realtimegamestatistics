...
import { AlertCircle, UserPlus, Timer, Zap, ArrowRightLeft } from 'lucide-react';
import { Button } from '../components/ui/button';

const substitutionRecommendations = [
  {
    id: 'rec-1',
    outgoing: { name: 'K. Walsh', id: 6, fatigue: 85, reason: 'Movement speed dropped by 12%.' },
    incoming: { name: 'G. Stanway', id: 14, warmUp: 'High' }
  },
  {
    id: 'rec-2',
    outgoing: { name: 'L. Hemp', id: 11, fatigue: 78, reason: 'Sprint recovery time increasing.' },
            ...
            <CardHeader className="p-6 pb-2">
              <div className="flex items-center justify-between">
                 <div className="flex items-center gap-2">
                    ...
                    <CardTitle className="text-lg font-black uppercase tracking-tight text-white">Sub Recommendations</CardTitle>
                 </div>
                 <Badge className="bg-zinc-200 text-zinc-900 border-none text-[9px] font-black uppercase">Tactical</Badge>
              </div>
            </CardHeader>
            <CardContent className="p-6 pt-2 space-y-4">
               {substitutionRecommendations.map((rec) => (
                 <div key={rec.id} className="border border-zinc-800 bg-zinc-900/50 rounded-2xl p-4 space-y-4">
                    <div className="flex items-center justify-between">
                       <div className="flex items-center gap-3">
                          <div className="flex flex-col items-center">
                             <span className="text-[10px] font-black text-zinc-500 uppercase">OUT</span>
                             <div className="w-8 h-8 rounded-lg bg-zinc-800 border border-zinc-700 flex items-center justify-center font-black text-zinc-100 text-xs mt-1">{rec.outgoing.id}</div>
                          </div>
                          <ArrowRightLeft className="w-4 h-4 text-zinc-600 mt-4" />
                          <div className="flex flex-col items-center">
                             <span className="text-[10px] font-black text-zinc-500 uppercase">IN</span>
                             <div className="w-8 h-8 rounded-lg bg-emerald-900/30 border border-emerald-800/50 flex items-center justify-center font-black text-emerald-500 text-xs mt-1">{rec.incoming.id}</div>
                          </div>
                       </div>
                       <div className="text-right">
                          <div className="text-xs font-black text-white uppercase">{rec.outgoing.name} → {rec.incoming.name}</div>
                          <div className="text-[9px] font-bold text-red-500 uppercase mt-0.5">{rec.outgoing.fatigue}% Fatigue</div>
                       </div>
                    </div>
                    ...
                    <Button variant="outline" className="w-full border-zinc-700 text-zinc-100 hover:bg-zinc-800 hover:text-white rounded-xl h-10 text-[10px] font-black uppercase tracking-widest">
            ...
            <CardContent className="p-0">
              <div className="divide-y divide-zinc-800">
                {[
                  ...
                  { id: 19, name: 'B. Mead', pos: 'FW', warm: 'Ready', energy: 100 },
                  ...
                  <div key={bench.id} className="p-4 flex items-center justify-between hover:bg-zinc-800/50 transition-colors">
                    ...
                    <Badge className="bg-zinc-800 text-zinc-300 border-zinc-700 text-[9px] font-black uppercase">{bench.warm}</Badge>
                  </div>
                ))}
              </div>
            </CardContent>
